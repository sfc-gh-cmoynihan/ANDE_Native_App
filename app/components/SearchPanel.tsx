...
interface SearchPanelProps {
  onCustomerSelect?: (id: string, name: string, email: string) => void
}
...
export function SearchPanel({ onCustomerSelect }: SearchPanelProps) {
  ...
  const selectCompany = async (company: string) => {
    ...
    setLoading(false)
    if (data.length > 0) {
      onCustomerSelect?.(data[0].MASTER_CUSTOMER_ID, data[0].FULL_NAME, data[0].EMAIL || "")
    }
  }
  ...
  const handleRowClick = (r: SearchResult) => {
    ...
    onCustomerSelect?.(r.MASTER_CUSTOMER_ID, r.FULL_NAME, r.EMAIL || "")
