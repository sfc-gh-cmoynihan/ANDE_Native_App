...
import { Search, Phone, Bot, LayoutDashboard, AlertCircle, Car, Activity, ShieldAlert, Fingerprint, Globe, TrendingUp, ShieldCheck } from "lucide-react"
...
const FraudPanel = lazy(() => import("@/components/FraudPanel").then(m => ({ default: m.FraudPanel })))
...
const GeoSpatialPanel = lazy(() => import("@/components/GeoSpatialPanel").then(m => ({ default: m.GeoSpatialPanel })))
const PredictiveModelingPanel = lazy(() => import("@/components/PredictiveModelingPanel").then(m => ({ default: m.PredictiveModelingPanel })))
const GreenFlagPanel = lazy(() => import("@/components/GreenFlagPanel").then(m => ({ default: m.GreenFlagPanel })))
...
const NAV_ITEMS = [
  ...
  { id: "greenflag", label: "Green Flag", icon: ShieldCheck },
...
export default function Home() {
  ...
  const handleCustomerSelect = (id: string, name: string, email: string) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-primary)" }}>
      <header style={{
        ...
        <span style={{ fontSize: 11, color: "#64748b", fontFamily: "monospace", whiteSpace: "nowrap" }}>v6.22</span>
      </header>
      <main style={{ padding: 24, maxWidth: 1400, margin: "0 auto" }}>
        <Suspense fallback={<LoadingSpinner />}>
          ...
          {activeTab === "contracts" && <ContractsPanel customerId={selectedCustomerId} customerName={selectedCustomerName} customerEmail={selectedCustomerEmail} />}
          {activeTab === "claims" && <DocumentSearchPanel customerId={selectedCustomerId} customerName={selectedCustomerName} customerEmail={selectedCustomerEmail} />}
          {activeTab === "calls" && (
            <>
          ...
          {activeTab === "predictions" && <PredictiveModelingPanel />}
          {activeTab === "greenflag" && <GreenFlagPanel />}
          {activeTab === "geospatial" && <GeoSpatialPanel />}
